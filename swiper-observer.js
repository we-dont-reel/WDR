...
function initSwiperObserver(){
    swiperObserver = new IntersectionObserver(handleSwiperObs, {
        threshold: 0,
        rootMargin: '50% 0px 50% 0px'
    });
    document.querySelectorAll('.obsi').forEach(function(x){
        swiperObserver.observe(x)
    });
}
...
function buildSwiper(el){
    el.dataset.active = "1";
    var c = el.className;
    if(c.indexOf('outid-') === -1 || c.indexOf('tot-img-') === -1){
        el.dataset.active = "";
        return;
    }
